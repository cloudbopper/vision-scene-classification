...
% load dictionary
inFName = fullfile(dataBaseDir, sprintf('dictionary_%d_%d_%s', ...
    params.gridSpacing, params.dictionarySize, params.dictionaryType));
load(inFName, 'dictionary');
fprintf('Loaded %s dictionary: %d codewords\n', params.dictionaryType, params.dictionarySize);
...
for f = 1:length(imageFileList)
    ...
    outFName = fullfile(dataBaseDir, sprintf('%s_encoding_%d_%d_%s.mat', baseFName, params.gridSpacing, params.dictionarySize, params.dictionaryType));
